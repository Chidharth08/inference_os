"""E001-B — Output Length (Decode) Scaling Benchmark Execution Script."""
...
def create_parser() -> argparse.ArgumentParser:
    """Create command-line argument parser for E001-B benchmark."""
    parser = argparse.ArgumentParser(
        description="E001-B — Output Length (Decode) Scaling Benchmark",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/e001b_decode_scaling.yaml",
        help="Path to YAML/JSON configuration file for E001-B",
    )
    parser.add_argument(
        "--base-url",
        type=str,
...
def print_sweep_table(points: list[dict[str, Any]]) -> None:
    ...
    print("=" * 110)
    print(" E001-B: OUTPUT LENGTH (DECODE) SCALING RESULTS SUMMARY")
    print("=" * 110)
    header = (
        f"{'Output Tokens':<14} | {'Status':<8} | {'E2E (P50)':<12} | "
        f"{'E2E (Mean)':<12} | {'TPOT (P50)':<12} | {'TTFT (P50)':<12} | "
    ...
    for pt in sorted(points, key=lambda x: x["param_value"]):
        ...
        row = (
            f"{p_val:<14} | {status_str:<8} | {e2e_p50_str:<12} | "
            f"{e2e_mean_str:<12} | {tpot_p50_str:<12} | {ttft_p50_str:<12} | "
...
async def main_async(args: argparse.Namespace) -> int:
    """Async main entrypoint for E001-B benchmark."""
    ...
    print("=" * 75)
    print(" inference_os — E001-B Output Length (Decode) Scaling Experiment")
    print("=" * 75)
    ...
    print(f" Fixed Input Tokens:  {sweep_config.base_config.prompt_tokens}")
...
def main() -> int:
    """CLI entry point for running E001-B benchmark."""
